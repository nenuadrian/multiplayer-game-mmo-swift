...
extension ByteManipulations {
  public func toByteArray() -> [UInt8] {
    var value: Self = self
    return withUnsafeBytes(of: &value) { Array($0) }
  }

  public static func fromByteArray(_ value: [UInt8]) -> Self {
    return value.withUnsafeBytes {
      $0.baseAddress!.load(as: Self.self)
    }
  }

  public static func fromUnsafePointer(_ value: UnsafePointer<UInt8>) -> Self {
    return fromByteArray(value.toArray(offset: 0, length: sizeOf()))
  }

  public static func sizeOf() -> Int {
...
extension UnsafePointer where Pointee == UInt8 {
  public func getString(lengthOffsetPosition: Int) -> String? {
    let length = Int(self[lengthOffsetPosition])
    let str =
      NSString(
        bytes: self + lengthOffsetPosition + 1, length: length,
        encoding: String.Encoding.utf8.rawValue) as String?
    return str
  }

  public func toArray(offset: Int, length: Int) -> [UInt8] {
...
open class SocketHandler {
  ...
  public var packetHandlers: [Packets: (_: UnsafePointer<UInt8>) -> Void] = [:]
  ...
  public func send(type: Packets, buff: [UInt8] = [UInt8]()) {
    do {
      let packetLength = UInt16(buff.count + 1)
      let packet = packetLength.toByteArray() + [type.rawValue] + buff
    ...
    queue.async { [unowned self, socket] in
      ...
      do {
          ...
          if bytesRead > 0 {
            readData.withUnsafeBytes { (bytes: UnsafePointer<UInt8>) -> Void in
              ...
              repeat {
                ...
                let rawOp = bytes[offset + 2]
                if let op = Packets.init(rawValue: rawOp) {
                  ...
                  Logger.debug("Unknown packet op \(rawOp)")
                }
                offset += Int(length) + 2
        ...
        guard error is Socket.Error else {
          Logger.debug(
            "Unexpected error by connection at \(self.socket.remoteHostname):\(self.socket.remotePort)..."
          )
          return
        }
      }
